...
```
    ...
    a.    
```{python}
# Count the number of unique providers
val_count_2016 = len(df_pos2016_short["PRVDR_NUM"].value_counts())
print()
```
...
# Count the number of unique providers, including 2016
val_count = [val_count_2016, len(df_pos2017_short["PRVDR_NUM"].value_counts()), len(
    df_pos2018_short["PRVDR_NUM"].value_counts()), len(df_pos2019_short["PRVDR_NUM"].value_counts())]

num_short_term = pd.DataFrame([val_count])
num_short_term.columns = list(range(2016, 2020, 1))
...
chart_short = alt.Chart(df_pos_short).mark_line().encode(
    alt.X("DATA_YEAR:N", title="Subset Year (Q4)", axis=alt.Axis(labelAngle=0)),
